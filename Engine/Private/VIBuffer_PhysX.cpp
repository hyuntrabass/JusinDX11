...
HRESULT CVIBuffer_PhysX::Init_Prototype(const PxRenderBuffer& Buffer)
{
	...
	m_iNumVertices = Buffer.getNbLines() * 2;
...
#pragma region Vertex
	...
	_uint iIndex{};
	for (size_t i = 0; i < Buffer.getNbLines(); i++)
	{
		pVertices[iIndex].vPosition.x = Buffer.getLines()[i].pos0.x;
		pVertices[iIndex].vPosition.y = Buffer.getLines()[i].pos0.y;
		pVertices[iIndex].vPosition.z = Buffer.getLines()[i].pos0.z;
		
		pVertices[iIndex].vColor.x = (Buffer.getLines()[i].color0 & 0xff) / 255.f;
		pVertices[iIndex].vColor.y = ((Buffer.getLines()[i].color0 >> 8) & 0xff) / 255.f;
		pVertices[iIndex].vColor.z = ((Buffer.getLines()[i].color0 >> 16) & 0xff) / 255.f;
		
		iIndex++;
		pVertices[iIndex].vPosition.x = Buffer.getLines()[i].pos1.x;
		pVertices[iIndex].vPosition.y = Buffer.getLines()[i].pos1.y;
		pVertices[iIndex].vPosition.z = Buffer.getLines()[i].pos1.z;
		
		pVertices[iIndex].vColor.x = (Buffer.getLines()[i].color1 & 0xff) / 255.f;
		pVertices[iIndex].vColor.y = ((Buffer.getLines()[i].color1 >> 8) & 0xff) / 255.f;
		pVertices[iIndex].vColor.z = ((Buffer.getLines()[i].color1 >> 16) & 0xff) / 255.f;
